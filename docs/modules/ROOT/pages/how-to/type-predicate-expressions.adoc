...
A type predicate expression serves to validate the type of a variable, literal, property, or another Cypher expression.
It follows the syntax below in Cypher:
...
The `isType` function takes a xref:variables-and-params/variables.adoc[Cypher variable] and a type specified in `Cypher.TYPE`:
