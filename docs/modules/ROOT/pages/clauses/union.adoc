...
It is possible to explicitly set the default behaviour of unions with `.distinct()` to create `UNION DISTINCT` clause.

=== Combine Union and Union All

If a `UNION`, or `UNION ALL` clause is passed as a clause to another `UNION`, it will be enclosed in curly braces to preserve preference.
...
const unionQuery = new Cypher.Union(new Cypher.Union(query1, query2), query3).all();
----
