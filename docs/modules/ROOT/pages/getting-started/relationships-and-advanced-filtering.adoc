...
:description: This page shows you how to add relationships and more advanced filters, and how to alias the results of a projection.
= Relationships and advanced filtering

This page shows you how to add relationships and more advanced filters, and how to alias the results of a projection
It extends the Cypher query example from xref:getting-started/filters-and-projections.adoc[Filters and projections] based link:https://neo4j.com/docs/getting-started/appendix/example-data/#built-in-examples[movies data set].
...
. Use `new Cypher.Pattern` to define an arbitrarily complex pattern with the variables already declared:
+
[source, javascript]
----
...
With these changes, the script yields the following result:
...
As advanced queries grow, they can start to hinder maintenance.
To avoid this, it is advisable to split the query into more basic comparisons and compose it afterward.
...
You can use `titleProp` and `yearProp` to xref:getting-started/filters-and-projections.adoc#_reusing_variables[reuse variables].
...
. Run the script. 
The resulting Cypher looks like this:
+
[source, cypher]
----
MATCH (this0:Movie)<-[this1:ACTED_IN]-(this2:Person)
...
[NOTE]
====
Make sure to double-check whether all variables refer to the correct parameter and node or relationship. 
====
