...
export class Pattern extends PatternElement {
    ...
    constructor(nodeConfig: NodeRef | NodePattern, previous?: PartialPattern) {
        let node = nodeConfig instanceof NodeRef ? nodeConfig : nodeConfig.variable;
        if (!node) {
            node = new NodeRef();
        }
        super(node);
        ...
        if (!(nodeConfig instanceof NodeRef)) {
            this.labels = nodeConfig.labels;
    ...
    public related(rel?: RelationshipRef | RelationshipPattern): PartialPattern {
        if (rel instanceof RelationshipRef) {
